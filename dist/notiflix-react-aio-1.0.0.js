...
const notiflixGoogleFont = function () {
    if (!document.getElementById('NotiflixQuicksand')) {
        ...
        dns.id = 'NotiflixGoogleDNS';
        dns.rel = 'dns-prefetch';
        dns.href = '//fonts.googleapis.com';
        document.head.appendChild(dns);

        const link = document.createElement('link');
        link.id = 'NotiflixQuicksand';
        link.rel = 'stylesheet';
...
// notify default settings off

// report default settings on
let reportSettings = {
...
// report default settings off

// confirm default settings on
let confirmSettings = {
...
// confirm default settings off

// loading default settings on
let loadingSettings = {
...
const Notiflix = {
    ...
    Notify: {
        ...
        Merge: function (userNotifyExtend) {
            ...
            if (newNotifySettings) { // if initied already
                newNotifySettings = extendNotiflix(true, newNotifySettings, userNotifyExtend);
            } else { // pretend like init
                newNotifySettings = extendNotiflix(true, notifySettings, userNotifyExtend);
            }
        },
    ...
    Report: {
        ...
        Merge: function (userReportExtend) {
            ...
            if (newReportSettings) { // if initied already
                newReportSettings = extendNotiflix(true, newReportSettings, userReportExtend);
            } else { // pretend like init
                newReportSettings = extendNotiflix(true, reportSettings, userReportExtend);
            }
        },
        ...
        Success: function (title, message, buttonText, buttonCallback) {
            ...
            if (arguments.length > 4) { // More parameters than allowed
                console.error('Notiflix Error: More parameters than allowed.');
                return;
            }
    ...
    Confirm: {
        ...
        Merge: function (userConfirmExtend) {
            ...
            if (newConfirmSettings) { // if initied already
                newConfirmSettings = extendNotiflix(true, newConfirmSettings, userConfirmExtend);
            } else { // pretend like init
                newConfirmSettings = extendNotiflix(true, confirmSettings, userConfirmExtend);
            }
        },

        // Display Confirm: Show
        Show: function (title, message, okText, cancelText, okCallback) {

            if (arguments.length > 5) { // More parameters than allowed
                console.error('Notiflix Error: More parameters than allowed.');
                return;
            }
            ...
            if (arguments[4] === undefined) { // buttonCallback
                arguments[4] = undefined;
            }

            NotiflixConfirm(arguments[0], arguments[1], arguments[2], arguments[3], arguments[4]);
        },
    },
    // Confirm off
    ...
    Loading: {
        ...
        Merge: function (userLoadingExtend) {
            ...
            if (newLoadingSettings) { // if initied already
                newLoadingSettings = extendNotiflix(true, newLoadingSettings, userLoadingExtend);
            } else { // pretend like init
                newLoadingSettings = extendNotiflix(true, loadingSettings, userLoadingExtend);
            }
        },
        ...
        Remove: function (theDelay) {
            ...
            if (arguments.length > 1) { // More parameters than allowed
                console.error('Notiflix Error: Parameters not allowed.');
                return;
            }
...
const NotiflixNotify = function (message, callback, theType, staticType) {
    ...
    if (arguments !== undefined && arguments.length === 4) {
        ...
        // use GoogleFonts on
        if (newNotifySettings.useGoogleFont) {
        ...
        console.error('Notiflix Error: Where is the arguments?');
    }
...
const NotiflixReport = function (title, message, buttonText, buttonCallback, theType, staticType) {
    ...
    // use GoogleFonts on
    if (newReportSettings.useGoogleFont) {
...
const NotiflixConfirm = function (title, message, okButtonText, cancelButtonText, okButtonCallback) {
    ...
    // use GoogleFonts on
    if (newConfirmSettings.useGoogleFont) {
    ...
    if (!document.getElementById(ntflxConfirmWrap.id)) {
        ...
        if (okButtonCallback) {  // if have a callback
        ...
        } else { // if ok button has not a callback
...
const NotiflixLoading = function (message, iconType, display, theDelay) {
    ...
    if (display) { // show it
        ...
        if (iconType === 'standard') {
            ...
            console.error('Notiflix Error: You have to set a static SVG url to "customSvgUrl" option to use Loading Custom.');
            return false;
        ...
        // use GoogleFonts on
        if (newLoadingSettings.useGoogleFont) {
...
const NotiflixLoadingChange = function (newMessage) {
    ...
    if (document.getElementById(loadingSettings.ID)) { // if has any loading
        ...
        if (newMessage.length > 0) { // if has any message
            ...
            console.error('Notiflix Error: Where is the new message?');
        }
